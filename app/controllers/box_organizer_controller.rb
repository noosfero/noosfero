class BoxOrganizerController < ApplicationController
  ...
  def add_block
    ...
    if ! type.blank?
      ...
      @center_block_types = (Box.acceptable_center_blocks & available_blocks) + plugins.dispatch(:extra_blocks, :type => :all, :position => 1)
      @side_block_types = (Box.acceptable_side_blocks & available_blocks) + plugins.dispatch(:extra_blocks, :type => :all, :position => [2,3])
