module BlockHelper
  ...
  def highlights_block_config_image_fields(block, image = {}, row_number = nil)
    ...
    <tr class=\"image-data-line\" data-row-number='#{row_number}'>
      ...
      <td colspan=\"3\"><label>#{content_tag('span', _('Title')) +
        text_field_tag('block[block_images][][title]', image[:title], class: 'highlight-title', size: 45)}</label></td>
      <td>#{button_without_text(:trash, _('Remove'), '#', class: 'delete-highlight', data: { confirm: _('Are you sure you want to remove this highlight') })}</td>
    </tr>
    ".html_safe
  end
end
