...
class ForumHelperTest < ActiveSupport::TestCase
  ...
  should 'list posts with different classes' do
    forum.children << older_post = create(TextileArticle, :name => 'First post', :profile => profile, :parent => forum, :published => false, :last_changed_by => profile)
    one_month_later = Time.now + 1.month
    Time.stubs(:now).returns(one_month_later)
    forum.children << newer_post = create(TextileArticle, :name => 'Second post', :profile => profile, :parent => forum, :published => true, :last_changed_by => profile)
