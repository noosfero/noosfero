...
class OrganizationMailingTest < ActiveSupport::TestCase
  ...
  should 'deliver mailing to each member after create' do
    create(OrganizationMailing, :source => community, :subject => 'Hello', :body => 'We have some news', :person => person)
    process_delayed_job_queue
    assert_equal community.members.compact.count, ActionMailer::Base.deliveries.count
  end
  ...
  should 'deliver mailing when there are many mailings created' do
    ...
    process_delayed_job_queue
    assert_equal 50*community.members.compact.count, ActionMailer::Base.deliveries.count
  end
