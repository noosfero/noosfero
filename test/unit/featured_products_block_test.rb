...
class FeaturedProductsBlockTest < ActiveSupport::TestCase
  ...
  should 'refer to products' do
    ...
    3.times {|n| products.push(create(Product, :name => "product #{n}", :enterprise_id => profile.id, :product_category_id => category.id)) }
  ...
  should "an environment block collect product automatically" do
    ...
    category = fast_create(ProductCategory)
    3.times {|n|
      create(Product, :name => "product #{n}", :enterprise_id => enterprise.id,
  ...
  should "an environment block collect just product with image automatically" do
    ...
    category = fast_create(ProductCategory)
    3.times {|n|
      create(Product, :name => "product #{n}", :enterprise_id => enterprise.id, :highlighted => true, :product_category_id => category.id)
    }
    @environment.boxes.first.blocks<< block
  ...
  should "an environment block collect just highlighted product automatically" do
    ...
    category = fast_create(ProductCategory)
    3.times {|n|
      create(Product, :name => "product #{n}", :enterprise_id => enterprise.id, :product_category_id => category.id, :image_builder => {
  ...
  should "return just highlighted products with image for selection" do
    ...
    products = []
    3.times {|n|
      products.push(create(Product, :name => "product #{n}", :enterprise_id => enterprise.id,
        :highlighted => true, :product_category_id => category.id,
        :image_builder => { :uploaded_data => fixture_file_upload('/files/rails.png', 'image/png') }
      ))
    }
    create(Product, :name => "product 4", :enterprise_id => enterprise.id, :product_category_id => category.id, :highlighted => true)
    create(Product, :name => "product 5", :enterprise_id => enterprise.id, :product_category_id => category.id, :image_builder => {
        :uploaded_data => fixture_file_upload('/files/rails.png', 'image/png')
      })
    @environment.boxes.first.blocks<< block
