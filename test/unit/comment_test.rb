...
class CommentTest < ActiveSupport::TestCase
  ...
  should "return activities comments as a thread" do
    person = fast_create(Person)
    a = create(TextileArticle, :profile => person, :name => 'My article', :body => 'Article body')
    c0 = create(Comment, :source => a, :body => 'My comment', :author => person)
    c1 = create(Comment, :reply_of_id => c0.id, :source => a, :body => 'bla', :author => person)
    c2 = create(Comment, :reply_of_id => c1.id, :source => a, :body => 'bla', :author => person)
    c3 = create(Comment, :reply_of_id => c0.id, :source => a, :body => 'bla', :author => person)
    c4 = create(Comment, :source => a, :body => 'My comment', :author => person)
