...
class CommentNotifierTest < Test::Unit::TestCase
  ...
  def setup
    ...
    @profile2 = create_user('user_comment_test2').person
  ...
  should 'deliver mail to owner of article with a another person' do
    @article.comments << Comment.new(:author => @profile2, :title => 'test comment', :body => 'you suck!')
  ...
  should 'include comment title in the e-mail' do
    @article.comments << Comment.new(:author => @profile2, :title => 'comment title', :body => 'comment title')
    sent = ActionMailer::Base.deliveries.first
    assert_match /comment title/, sent.body
  end

  should 'include comment text in the e-mail' do
    @article.comments << Comment.new(:author => @profile2, :title => 'comment title', :body => 'comment body')
    sent = ActionMailer::Base.deliveries.first
  ...
  should 'not deliver mail to comments author' do
    ...
    community.add_admin @profile2

    article = fast_create(Article, :name => 'Article test', :profile_id => community.id, :notify_comments => true)
    article.comments << Comment.new(:author => @profile, :title => 'test comment', :body => 'there is no addresses to send notification')
    sent = ActionMailer::Base.deliveries.first
    assert_not_includes sent.to, @profile.email
  end
