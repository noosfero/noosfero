...
class BoxesHelperTest < ActionView::TestCase
  ...
  should 'display embed button when a block is embedable' do
    ...
    assert_tag_in_string block_edit_buttons(block), :tag => 'a',
      :attributes => {:class => 'button icon-code without-text '}
  end
  ...
  should 'not display embed button when a block is not embedable' do
    ...
    stubs(:url_for).returns('')
    @controller.stubs(:user).returns(box.owner)
    assert_no_tag_in_string block_edit_buttons(block), :tag => 'a', :attributes => {:class => 'button icon-button icon-code '}
  end
