...
class OpenGraphPlugin::Activity < OpenGraphPlugin::Track
  ...
  def trackers
    @trackers ||= begin
      ...
      if self.defs[:passive]
        ...
        return unless self.object_actor && self.object_actor.person?

        actor = self.custom_actor || self.actor

        match_track = self.track_configs.any? do |c|
          c.enabled?(self.context, actor) &&
  ...
  protected
    ...
    def recent_publish?(actor, object_type, object_data_url)
      ...
      activity.present? && (activity.created_at <= self.update_delay.from_now)
    end

    def call(p, *args)
      p && instance_exec(*args, &p)
    end
end
