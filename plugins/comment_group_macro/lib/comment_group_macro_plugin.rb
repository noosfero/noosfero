...
class CommentGroupMacroPlugin < Noosfero::Plugin
  ...
  def macro_display_comments(params, inner_html, source)
    ...
    lambda {render :partial => 'plugins/comment_group_macro/views/comment_group.rhtml', :locals => {:group_id => group_id, :article_id => article.id, :inner_html => inner_html, :count => count }}
  end
  ...
  def comment_form_extra_contents(args)
    lambda {
      hidden_field_tag('comment[group_id]', args[:group_id]) if args.has_key?(:group_id)
    }
  end
