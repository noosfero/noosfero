...
class RangeTest < ActiveSupport::TestCase
  ...
  should 'get ranges of a metric configuration' do
    ...
    Kalibro::Range.expects(:request).with("Range", :ranges_of, {:metric_configuration_id => metric_configuration_id}).returns({:range => [@hash]})
  ...
  should 'return true when range is saved successfully' do
    ...
    Kalibro::Range.expects(:request).with("Range", :save_range, {:range => @created_range.to_hash, :metric_configuration_id => metric_configuration_id}).returns(:range_id => id_from_kalibro)
  ...
  should 'return false when range is not saved successfully' do
    metric_configuration_id = 2
    Kalibro::Range.expects(:request).with("Range", :save_range, {:range => @created_range.to_hash, :metric_configuration_id => metric_configuration_id}).raises(Exception.new)
    assert !(@created_range.save(metric_configuration_id))
    assert_nil @created_range.id
  end

  should 'destroy range by id' do
    Kalibro::Range.expects(:request).with("Range", :delete_range, {:range_id => @range.id})
    @range.destroy
  end
