...
class MetricConfigurationTest < ActiveSupport::TestCase
  ...
  should 'get all metric configurations of a configuration' do
    ...
    Kalibro::MetricConfiguration.expects(:request).with("MetricConfiguration", :metric_configurations_of, request_body).returns(response_hash)    
  ...
  should 'return true when metric configuration is saved successfully' do
    ...
    Kalibro::MetricConfiguration.expects(:request).with("MetricConfiguration", :save_metric_configuration, {:metric_configuration => @created_metric_configuration.to_hash, :configuration_id => configuration_id}).returns(:metric_configuration_id => id_from_kalibro)
  ...
  should 'return false when metric configuration is not saved successfully' do
    configuration_id = @created_metric_configuration.configuration_id
    Kalibro::MetricConfiguration.expects(:request).with("MetricConfiguration", :save_metric_configuration, {:metric_configuration => @created_metric_configuration.to_hash, :configuration_id => configuration_id}).raises(Exception.new)
    assert !(@created_metric_configuration.save)
    assert_nil @created_metric_configuration.id
  end

  should 'destroy metric configuration' do
    Kalibro::MetricConfiguration.expects(:request).with("MetricConfiguration", :delete_metric_configuration, :metric_configuration_id => @native_metric_configuration.id)
