  ...
  module ClassMethods
    ...
    def sync_serialized_field field, &block
      ...
      if not self.method_defined? field_data and field_data.to_s.in? self.column_names
        ...
        alias_method :field_data_without_sync, :field_data
        alias_method :field_data, :field_data_with_sync
      end
      if not self.method_defined? "#{field_data}=" and field_data.to_s.in? self.column_names
        ...
        alias_method ("#{field_data}="_without_prepare).to_sym, :("#{field_data}=").to_sym
        alias_method :("#{field_data}=").to_sym, ("#{field_data}="_with_prepare).to_sym
      end
