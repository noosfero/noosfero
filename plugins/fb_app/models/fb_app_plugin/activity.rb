class FbAppPlugin::Activity < OpenGraphPlugin::Activity
  ...
  def publish!(actor = self.actor)
    ...
    return if auth.blank? || auth.expired?
    ...
    return if (self.defs[:on] == :update) && self.recent_publish?(actor, self.object_type, self.object_data_url)
