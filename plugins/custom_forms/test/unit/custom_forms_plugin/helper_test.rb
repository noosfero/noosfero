...
class CustomFormsPlugin::HelperTest < ActiveSupport::TestCase
  ...
  should 'get proper time status' do
    ...
    s1 = CustomFormsPlugin::Form.create!(:profile => profile, :name => 'Survey 1', :identifier => 'survey-1', :beginning => Time.now + 1.day, :ending => Time.now + 2.days)
    s2 = CustomFormsPlugin::Form.create!(:profile => profile, :name => 'Survey 2', :identifier => 'survey-2', :beginning => Time.now - 1.day, :ending => Time.now + 1.day)
    s3 = CustomFormsPlugin::Form.create!(:profile => profile, :name => 'Survey 3', :identifier => 'survey-3', :beginning => Time.now - 2.days, :ending => Time.now - 1.day)
    s4 = CustomFormsPlugin::Form.create!(:profile => profile, :name => 'Survey 4', :identifier => 'survey-4', :beginning => Time.now + 1.day)
    s5 = CustomFormsPlugin::Form.create!(:profile => profile, :name => 'Survey 5', :identifier => 'survey-5', :beginning => Time.now - 1.day)
