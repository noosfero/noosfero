...
Dir.glob(Rails.root.join(plugins_root, '*', 'controllers')) do |controllers_dir|
  ...
  controllers_by_folder.each do |folder, controllers|
    controllers.each do |controller|
      ...
      if %w[profile myprofile].include?(folder.to_s)
        match "#{prefixes_by_folder[folder]}/#{plugin_name}/#{controller_name}(/:action(/:id))", controller: controller, profile: /#{Noosfero.identifier_format}/, via: :all
      else
  ...
  match 'profile/:profile/plugin/' + plugin_name + '(/:action(/:id))', controller: plugin_name + '_plugin_profile', profile: /#{Noosfero.identifier_format}/, via: :all
  match 'myprofile/:profile/plugin/' + plugin_name + '(/:action(/:id))', controller: plugin_name + '_plugin_myprofile', profile: /#{Noosfero.identifier_format}/, via: :all
